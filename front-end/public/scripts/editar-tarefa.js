...
const inputsList = [titleInput, costInput, descriptionInput];
...
window.addEventListener('load', function getTask(){
    ...
    fetch(urlToGetTaskById, init)
        ...
        .then(json => {
            ...
            hourInput.setAttribute('placeholder', splittedHourly[0])
            minInput.setAttribute('placeholder', splittedHourly[1])

            inputsList.forEach(input => input.setAttribute('placeholder', json[input.name]))
        });
})
